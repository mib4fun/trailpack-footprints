...
module.exports = {
  ...
  getControllerFootprints(app) {
    if (_.get(app.config, 'footprints.controllers')) {
      const controllers = _.omit(
        app.controllers,
        _.get(app.config, 'footprints.controllers.ignore')
      )
  ...
  getModelFootprints(app) {
    ...
    const prefix = _.get(app.config, 'footprints.prefix')
    const plural = _.get(app.config, 'footprints.pluralize')

    console.log(lib.Routes)
    return _(lib.Routes)
      .filter(route => {
      ...
      .map(route => {
        route.path = plural ? pluralize(route.path) : route.path
